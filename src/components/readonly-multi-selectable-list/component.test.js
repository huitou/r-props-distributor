/*
    Read-only Multi-Selectable List Component Test.
...
import ReadonlyMultiSelectableListComponent from './component';
...
describe('ReadonlySelectableListComponent', () => {
    describe('when mounted with initialElements props,', () => {
        let wrapper
        beforeEach(() => {
            wrapper = shallow(<ReadonlyMultiSelectableListComponent initialElements={initialElements}/>);
        })
        ...
        it('has a selectElement handle which sets state.selectedElements only with one of state.elements', () => {
            ...
            expect(wrapper.state().selectedElements).toEqual([wrapper.instance().elements()[1]]);
        });
        ...
        it('has a selectElement handle which does not set state.selectedElements with a non-member of state.elements', () => {
            ...
            expect(wrapper.state().selectedElements).toEqual([]);
        });

        it('has a deselect handle which sets state.selectedElements to undefined', () => {
            wrapper.instance().selectElement(wrapper.instance().elements()[1]);
    ...
    describe('when mounted without initialElements props,', () => {
        let wrapper
        beforeEach(() => {
            wrapper = shallow(<ReadonlyMultiSelectableListComponent />);
        })
